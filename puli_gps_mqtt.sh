...
while true; do
    ...
    if echo "$RAW_DATA" | grep -q "+QGPSLOC:"; then
        ...
        CONVERTED=$(echo "$GPS_CSV" | awk -F, '{
            ...
            dec_lat = (substr(raw_lat, 1, 2) + (substr(raw_lat, 3) / 60)) * sgn_lat;
            dec_lon = (substr(raw_lon, 1, 3) + (substr(raw_lon, 4) / 60)) * sgn_lon;
        ...
        if [ -n "$CUR_LAT" ] && [ -n "$CUR_LON" ]; then
            ...
            if [ "$PUBLISH_TRIGGER" -eq 1 ]; then
                ...
                # Build valid JSON payload using clean decimal floats
    ...
    TIMER=$((TIMER + SLEEP_INTERVAL)) # Advance tracking clock
done
